...
const WeHave = ({ isLikeContent = false }: Props) => {
	...
	const transition = useMemo(() => (
		{
			enter: "transform transition-all ease-linear duration-700",
			enterFrom: type === "exp" || type === "challenges" ? "-translate-y-full opacity-0" : "translate-y-full opacity-0",
			enterTo: type === "exp" || type === "challenges" ? "-translate-y-20 opacity-50" : "translate-y-20 opacity-50",
			leave: "transform transition-all ease-linear duration-100",
			leaveFrom: type === "exp" || type === "challenges" ? "-translate-y-20 opacity-50" : "translate-y-20 opacity-50",
			leaveTo: type === "exp" || type === "challenges" ? "translate-y-full opacity-0" : "-translate-y-full opacity-0"
		}
	), [type]);
	...
	return (
		<div ref={gridRef} className={`relative h-[175vh] ${!isMobile && "text-center"}`}>
			<WhiteGrid type={type}>
				<div style={{background: "radial-gradient(closest-side, rgba(250,249,247,1) 50%, rgba(0,0,0,0) 100%)"}} className="mx-auto max-w-7xl py-24 px-12 lg:py-64 lg:px-32 relative">
					<Transition
						show={type === "exp"}
						{...transition}
					>
						{!isLikeContent && <div className="flex flex-col gap-8 max-w-5xl mx-auto">
