...
const WeHave = () => {
	...
	return (
		<div ref={gridRef} className="relative h-[150vh]">
			<Grid type={type}>
				<div   style={{background: "radial-gradient(closest-side, rgba(250,249,247,1) 50%, rgba(0,0,0,0) 100%)"}} className="mx-auto max-w-7xl py-64 px-32">
					<Transition
						show={type === "exp"}
					...
					>
						{type === "exp" && <div className="flex flex-col gap-8 text-center max-w-5xl mx-auto">
							...
							<p className="font-extrabold text-xl">Nous pourrons vous conseiller sur votre stratégie produit et développement logiciel global</p>
						</div>}
					</Transition>
					<Transition
						show={type === "skill"}
